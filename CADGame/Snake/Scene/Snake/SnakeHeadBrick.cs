...
namespace CADGame
{
    public static class SnakeHeadBrick
    {
        private static string blockName = "SnakeHeadBrick";
        private static List<Entity> blockEnts = new List<Entity>(); //用于保存block中的各图元
        private static ObjectId blockId = ObjectId.Null;
        private static double lineLength = 10;
        private static double factor = 0.6;
        private static short colorIndex = 100;
        ...
        public static List<Entity> BlockEnts
        {
            get
            {
                ...
                for (double i = -0.5 * factor * lineLength; i < 0.5 * factor * lineLength; i += setp)
                {
                    ...
                    line.Color = Color.FromColorIndex(ColorMethod.ByColor, colorIndex); //颜色索引值
                    blockEnts.Add(line);
                }
                for (double i = -0.5 * factor * lineLength; i < 0.5 * factor * lineLength; i += setp)
                {
                ...
                return SnakeHeadBrick.blockEnts;
            }
            set { }
        }
